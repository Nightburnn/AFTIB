...
const Listing = () => {
  ...
  const loadListingDetails = async () => {
    const response = await axios.get('http://localhost:8080/listing/getListingById/' + edit_id);
    ...
    setFormValues({
      ...
      state: data?.state,
      LGA: data?.title,
      ownerName: data?.ownersContact ? data?.ownersContact.name : '',
      ownerPhone: data?.ownersContact ? data?.ownersContact.phone : '',
      ownerEmail: data?.ownersContact ? data?.ownersContact.email : '',
      virtualTour: data?.virtualTour,
      neighborhood: data?.neighborhood,
      propertyStatus: data?.propertyStatus,
      // Need to change this property
      energyRating: data?.title,
      nearbySchools: data?.nearbySchools,
      transportation: data?.transportation,
      floorNumber: data?.floorNumber,
      agentName: data?.agentContact ? data?.agentContact?.name : '',
      agentPhone: data?.agentContact ? data?.agentContact?.phone : '',
      agentEmail: data?.agentContact ? data?.agentContact?.email : '',
    })
  }
  ...
  async function updateForm() {
    ...
    if (!validate.valid && validate.message) alert(validate.message);
    if (!validate.valid) return;
    try {
      let addListing = await axios.put('http://localhost:8080/listing/updateListing/' + edit_id, JSON.stringify(requestBody), {
        headers: {
  ...
  async function submitForm() {
    ...
    try {
      // BEFORE 2024/06/27
      // let addListing = await axios.post('http://localhost:8080/listing/addListing', JSON.stringify(requestBody), {
      //   headers: {
      //     'Content-Type': 'application/json',
      //     "Authorization": `Bearer ${token}`
      //   }
      ...
      const result = await axios.put(`http://localhost:8080/listing/addListingImages/${addListing.data.listingId}`, formData, {
        headers: {
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="row justify-content-center">
        <button className="btn listbtn  mt-3" onClick={edit_id ? updateForm : submitForm}>
          {/* BEFORE 2024/06/27 */}
          {/* <Link to="/review" className='listbtn'>Submit</Link> */}
